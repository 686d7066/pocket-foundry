...
/** Copies runtime assets, excluding test directories; code and styles are compiled separately. */
export function isRuntimeAsset(path: string): boolean {
  return statSync(path).isDirectory() ? basename(path) !== "tests" : RUNTIME_ASSET_EXTENSIONS.has(extname(path).toLowerCase());
}
